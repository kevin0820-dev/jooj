...
// Unlimited Edit
	...
	if (not_empty($cl['post_data']) && $cl['post_data']["user_id"] == $me["id"] && not_empty($cl['post_data']["edited"])) {
	    ...
	    $data['status'] = 200;
        $data['html']   = cl_template('timeline/modals/edit_blue');
	}
}
// First Edit
else if ($action == 'get_data_blue_f') {
	$data['status']   = 400;
    $data['err_code'] = 0;
    $post_id          = fetch_or_get($_GET["id"], false);
	$post_id          = cl_text_secure($post_id);
	...
	if (not_empty($cl['post_data']) && $cl['post_data']["user_id"] == $me["id"] && empty($cl['post_data']["edited"])) {
	    ...
	    $data['status'] = 200;
        $data['html']   = cl_template('timeline/modals/edit_blue_f');
	}
}
// Save Unlimited Edit
else if ($action == 'save_data_blue') {
	$data['status']   = 400;
    $data['err_code'] = 0;
	...
	if (not_empty($post_data) && $post_data["user_id"] == $me["id"] && not_empty($post_data["edited"])) {
...
// Save First Edit
else if ($action == 'save_data_blue_f') {
	$data['status']   = 400;
    $data['err_code'] = 0;
    $post_id          = fetch_or_get($_POST["id"], false);
	$post_id          = cl_text_secure($post_id);
