...
if ($action == 'load_more') {
    ...
    if (is_posnum($prof_id) && is_posnum($offset) && in_array($type, array('watchers','following', 'follow_requests'))) {
        if (cl_can_view_profile($prof_id)) {
        	if ($type == 'watchers') {
        		$users_list = cl_get_watchers($prof_id, 30, $offset);	
        	}
        	...
        	if (not_empty($users_list)) {
    			foreach ($users_list as $cl['li']) {
    				$html_arr[] = cl_template('watchings/includes/list_item');
    			}
    ...
    if (is_posnum($req_id)) {
        $req_data = cl_db_get_item(T_WATCHERS, array("id" => $req_id));
    ...
    if (is_posnum($req_id)) {
        if (cl_db_delete_item(T_WATCHERS, array("id" => $req_id))) {
            $data['status'] = 200;
            $data['total']  = cl_get_follow_requests_total();
        }
    }
